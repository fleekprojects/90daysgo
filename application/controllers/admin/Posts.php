<?php
	...
	class Posts extends MY_Controller {
		...
		public function AddNew(){
			$this->Dmodel->checkLogin();
			$viewdata['title']=$this->pagetitle;
			$this->LoadAdminView("admin/addPost",$viewdata);
		}
		...
		public function Edit($id){
			...
			$this->LoadAdminView("admin/editPost",$viewdata);
		}
		...
		public function AddRecord(){
			...
			if($this->Dmodel->IFExist($this->table,'title',$data['title'])){
				...
				if(isset($_FILES['image']) && $_FILES['image']['tmp_name']){
					$config['upload_path']          = APPPATH.'../assets/front/uploads/posts';
					...
					if(file_exists(APPPATH.'../assets/front/uploads/posts/'.$ldata['image'])){
						unlink(APPPATH.'../assets/front/uploads/posts/'.$ldata['image']);
					}
		...
		public function DeleteRecord(){
			...
			foreach($ids as $id){
				$record=$this->Dmodel->get_table_where('image', $this->table, $id);
				$image_file=APPPATH.'..\assets\front\uploads\posts/'.$record[0]['image'];
				if(unlink($image_file)){
					$result=$this->Dmodel->delete_rec($id,$whr_key,$this->table);
				}
			}
