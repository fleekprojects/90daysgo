			...
			<table id="datatable-buttons" class="table table-striped table-bordered">
			  ...
			  ?>
				...
				<input class="chkIds" type="checkbox" name="ids[]" id="chk-<?= $rec['id'] ?>" value="<?= $rec['id'] ?>"  /></td>
				  <td><img src="<?=base_url();?>assets/front/uploads/posts/<?= ($rec['image'] != "" ? $rec['image'] : "dummy.png"); ?>"  style="max-height:50px;" /></td>
				  <td><?= ($rec['post_type']=='1'?'Blog':'Page'); ?></td>
				  <td><?= $rec['title']; ?></td>
				  <td><?= $rec['slug']; ?></td>
				  <td>
				   <div class="form-group">
					<label>
					  <input type="checkbox" class="js-switch"  <?= ($rec['status']==1) ? 'checked' : '' ?> onclick="togglestatus(<?= $rec['id'] ?>,'Posts')" />
					</label>
				  </div></td>
				  <td>
